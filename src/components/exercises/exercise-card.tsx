...
const difficultyCardStyles: Record<Difficulty, string> = {
  ...
  Avanzado: 'border-l-4 border-l-rose-500 bg-gradient-to-r from-rose-500/5 to-transparent hover:border-l-rose-400',
}
...
export function ExerciseCard({ exercise, isSolved }: ExerciseCardProps) {
  ...
  return (
    <Link href={`/exercises/${exercise.id}`} className="block h-full group">
      <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.15 }} className="h-full">
        <Card
          className={cn(
          ...
          <CardHeader className="p-4 pb-2">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <Badge
                  variant="outline"
                  className={cn('shrink-0 text-[10px] px-1.5 py-0', difficultyBadgeStyles[difficulty])}
                >
          ...
          <CardContent className="p-4 pt-0">
            ...
            <div className="flex items-center justify-end mt-3">
              <ArrowRight className={cn(
                ...
                difficulty === 'Principiante' && 'text-emerald-500/60 group-hover:text-emerald-500',
                difficulty === 'Intermedio' && 'text-amber-500/60 group-hover:text-amber-500',
                difficulty === 'Avanzado' && 'text-rose-500/60 group-hover:text-rose-500'
              )} />
            </div>
          </CardContent>
        </Card>
