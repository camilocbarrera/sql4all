  ...
  return (
    <div className="flex items-center gap-2 py-1 px-2 text-sm hover:bg-muted/50 rounded">
      ...
      <span
        className={cn(
          "font-mono",
          isPrimaryKey && "font-semibold text-amber-600 dark:text-amber-400",
        )}
      >
        {column.name}
      </span>
...
function TestQueryResults({
  results,
}: {
  results: DDLValidationResult["testQueryResults"];
}) {
          ...
          >
            ...
            {test.actual !== test.expected && (
              <div className="mt-1 text-xs opacity-75">
                Esperado: {test.expected ? "éxito" : "error"} | Resultado:{" "}
                {test.actual ? "éxito" : "error"}
              </div>
            )}
          </div>
        ))}
...
export function SchemaStateViewer({
  schemaInfo,
  validationResult,
  isLoading,
}: SchemaStateViewerProps) {
  if (isLoading) {
    return (
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {schemaInfo.tables.map((table) => (
          <TableCard key={table.name} table={table} isValid={tableValidity} />
        ))}
      </div>
