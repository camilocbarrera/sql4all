...
import { exercises, submissions } from "../src/lib/db/schema";
...
async function seedHeatmapData() {
  ...
  try {
    ...
    for (const pattern of activityPatterns) {
      ...
      while (
        activeDays.size < pattern.daysActive &&
        activeDays.size < daysInMonth
      ) {
      ...
      for (const day of activeDays) {
        const submissionsCount =
          Math.floor(Math.random() * pattern.maxPerDay) + 1;
    ...
    for (let i = 0; i < submissionsToInsert.length; i += batchSize) {
      ...
      console.log(
        `  ✓ Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(submissionsToInsert.length / batchSize)}`,
      );
    }
